...
def touch_calibration():
    ...
    for n in range(0, 4):
        while True:
            ...
            if x_original < 32700 and y_original > 100:
                img.draw_circle(
                    calibration_coordinate[n][0], calibration_coordinate[n][1], 4, fill=True)
                img.draw_circle(
                    calibration_coordinate[n][0], calibration_coordinate[n][1], 13, color=(255, 255, 0))
                img.draw_string(calibration_coordinate[n][0] - 4, calibration_coordinate[n][1] - 25, str(
                    3 - round((millis() - timer) / 1000)))
                if millis() - timer > 3000:
                    ...
                    img.draw_circle(
                        calibration_coordinate[n][0], calibration_coordinate[n][1], 4, fill=True)
                    img.draw_circle(
                        calibration_coordinate[n][0], calibration_coordinate[n][1], 13, color=(0, 255, 0))
                    lcd.display(img, x_size=320)
                ...
                img.draw_circle(
                    calibration_coordinate[n][0], calibration_coordinate[n][1], 4, fill=True)
                img.draw_circle(
                    calibration_coordinate[n][0], calibration_coordinate[n][1], 13, color=(255, 0, 0))
            img.draw_string(115, 110, 'screen calibration', mono_space=False)
        display(img)
        print(x_record)
        print(y_record)
    ...
    calibration_data = [x_min, x_max, x_step, y_min,
                        y_max, y_step, z_min, z_max, z_x_step, z_y_step]
...
def display(img):
    ...
    recv1 = spi.recv(1)
    recv2 = spi.recv(1)
    x_original = struct.unpack('B', recv1)[
        0] * 256 + struct.unpack('B', recv2)[0]
    ...
    recv1 = spi.recv(1)
    recv2 = spi.recv(1)
    y_original = struct.unpack('B', recv1)[
        0] * 256 + struct.unpack('B', recv2)[0]
    ...
    if x > -1 and y > -1:
        ...
        recv1 = spi.recv(1)
        recv2 = spi.recv(1)
        z_original = struct.unpack('B', recv1)[
            0] * 256 + struct.unpack('B', recv2)[0]
        z = calibration_data[6] + press_threshold - round(
            (z_original - x**1.02 * calibration_data[8] - y * 1.02 * calibration_data[9]))
        if z < 0:
            x = -1
            y = -1
            press = False
